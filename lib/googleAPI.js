...
function createJwt(payload, privateKey) {
    var header = encodeBase64Url(JSON.stringify({ typ: 'JWT', alg: 'RS256' }));
    var content = encodeBase64Url(JSON.stringify(payload));
    var signingInput = header + '.' + content;
    var signature = crypto.createSign('RSA-SHA256').update(signingInput).sign(privateKey);
