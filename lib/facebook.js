...
module.exports.validatePurchase = function (oneTimeAppAccessToken, receipt, cb) {
    ...
    var purchaseObj = '';
    try {
        sign = Buffer.from(encodedSign, 'base64url');
        purchase = Buffer.from(encodedPurchase, 'base64url');
    ...
    if (sign.length !== hmac.length || !crypto.timingSafeEqual(sign, hmac)) {
    ...
    var graphVersion = config.facebookApiVersion || 'v3.3';
    var graphUrl = 'https://graph.facebook.com/' + graphVersion + '/' + encodeURIComponent(purchaseObj.payment_id);
    request.get({ url: graphUrl, qs: requestParams, json: true }, function (error, response, paymentRes) {
        if (error || !response || response.statusCode >= 400 || !paymentRes || paymentRes.error) {
            verbose.log('<Facebook> Failed to call graph api: ', paymentRes);
            paymentRes = paymentRes || {};
            var graphError = error || paymentRes.error;
            cb(new Error('failed to validate purchase: graph api call error: ' + JSON.stringify(graphError)), {
                status: constants.VALIDATION.FAILURE,
                message: 'graph api call error: ' + JSON.stringify(graphError)
            });
            return;
        }
